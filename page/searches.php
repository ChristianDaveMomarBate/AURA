    ...
    if ($_GET["action"] == "delete") {
        foreach ($_SESSION["shopping_cart"] as $keys => $values) {
            if ($values["item_id"] == $_GET["id"]) {
                ...
                echo '<script>window.location="TLclientspage.php"</script>';
            }
        }
    }
}
                    ...
                    <div class="col-md-4 col-md-offset-4">
                        <div class="login-panel panel panel-default">
                            ...
                            <form role="form" action="" name="formR" method="post">
                                ...
                                <div class="form-group">
                                    <input class="form-control" placeholder="ENTER FULL NAME " name="name" type="text" autofocus required>
                                </div>
                                <div class="form-group">
                                    <input class="form-control" placeholder="PHONE NUMBER" name="phone" type="number" autofocus required>
                                </div>
                                <div class="form-group">
                                ...
                                ?>
                                        <!-- <a><?php #echo $values["item_name"]; 
                                                ?></a>-->
                                        <td align="right"><input type="hidden" class="form-control" id="" name="servicerequest" required value=" <?php echo $values["item_name"]; ?>"></td>
                                    <?php
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        ?>
                                                <tr>
                                                    ...
                                                    <td><a href="TLclientspage.php?action=delete&id=<?php echo $values["item_id"]; ?>"><span class="text-danger">Remove</span></a></td>
                                                </tr>
                                            <?php
                                                $total = $total + ($values["item_quantity"] * $values["item_price"]);
                                            }
                            ...
                            <div class="container">
                                <?php   
                                $bernz  = $_POST['essearch'];
                                echo ($bernz);
                                $query = "SELECT * FROM services where servicename like '%$bernz%' ORDER BY id ASC";
                                $result = mysqli_query($connect, $query);
                                if (mysqli_num_rows($result) > 0) {
                                    while ($row = mysqli_fetch_array($result)) {
                                ?>
                                        <form method="post" action="searches.php?action=add&id=<?php echo $row["id"]; ?>">
                                            <div class="col-md-4">
                                                <div style="border:1px solid #333; background-color:#f1f1f1; border-radius:5px; padding:16px;" align="center">
                                                    <img src="images/<?php echo $row["image"]; ?>" class="img-responsive" /><br />
                                                    <h4>-----------------------------</h4>
                                                    <h6>ESTABLISHMENT NAME :</h6>
                                                    <h4 class="text-info"><?php echo $row["maincategory"]; ?></h4>
                                                    <h4 class="text-info" style="color:red;"><?php echo $row["location"]; ?></h4>
                                                    <h4>------------------------------</h4>
