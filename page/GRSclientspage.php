    ...
    if ($_GET["action"] == "delete") {
        foreach ($_SESSION["shopping_cart"] as $keys => $values) {
            if ($values["item_id"] == $_GET["id"]) {
                ...
                echo '<script>window.location="GRSclientspage.php"</script>';
            }
        }
    }
}
                ...
                <div class="row">
                    <div class="col-md-4 col-md-offset-4">
                        <div class="login-panel panel panel-default">
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        ?>
                                                <tr>
                                                    ...
                                                    <td><a href="GRSclientspage.php?action=delete&id=<?php echo $values["item_id"]; ?>"><span class="text-danger">Remove</span></a></td>
                                                </tr>
                                            <?php
                                                $total = $total + ($values["item_quantity"] * $values["item_price"]);
                                            }
                            ...
                            <div class="container">
                                <?php
                                $holder = "GRAND-ROYAL-SPA" ;
                                $query = "SELECT * FROM services where maincategory like '%$holder%' ORDER BY id ASC";
                                $result = mysqli_query($connect, $query);
                                if (mysqli_num_rows($result) > 0) {
                                    while ($row = mysqli_fetch_array($result)) {
                                ?>
                                        <form method="post" action="GRSclientspage.php?action=add&id=<?php echo $row["id"]; ?>">
                                            <div class="col-md-4">
